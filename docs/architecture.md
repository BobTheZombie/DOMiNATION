...
- Interactive camera now supports a pitched perspective RTS rig (focus target + yaw + pitch + distance + FOV), WASD/arrow pan, edge scroll, middle-mouse drag pan, smooth distance interpolation, and bounded tactical/operational/strategic presets.
- Distance limits clamp at `[8,72]` for normal play and `[8,132]` for GOD mode; pitch/FOV are bounded for strategic readability.
